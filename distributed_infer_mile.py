...
if __name__ == "__main__":
    ...
    parser.add_argument('--config',
                        default='configs/MILE_infer.yaml',
                        type=str,
                        help="config path")
    ...
    bev_label_save_path = save_path + '/bev_label/'
    bev_label_pred_save_path = save_path + '/bev_label_pred/'
    offset_save_path = save_path + '/offset/'
    ...
    if rank == 0:
        ...
        if not os.path.exists(center_pred_save_path):
            os.makedirs(center_pred_save_path)
        if not os.path.exists(bev_label_save_path):
            os.makedirs(bev_label_save_path)
        if not os.path.exists(bev_label_pred_save_path):
            os.makedirs(bev_label_pred_save_path)
        if not os.path.exists(offset_save_path):
            os.makedirs(offset_save_path)
        if not os.path.exists(center_save_path):
            os.makedirs(center_save_path)
        if not os.path.exists(route_map_save_path):
            os.makedirs(route_map_save_path)
    dist.barrier()
    idx = 0
    with torch.no_grad():
