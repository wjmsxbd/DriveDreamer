...
class dataloader(data.Dataset):
    ...
    def get_data_info(self,idx):
        ...
        gt_trajectory = self.gt_trajectory_segment[idx]
        if gt_trajectory[-1][0] >= 2:
        ...
        for key in self.collect_condition:
            if key == '3Dbox':
                ...
                if self.use_original_action:
                    out[key] = torch.zeros((self.movie_len,12))
                else:    
                    out[key] = torch.zeros((self.movie_len,27))
            # elif key == 'bev_images':
        ...
        for i in range(self.movie_len):
            ...
            if self.use_original_action:
                ...
                now_action = torch.cat([actions[i],command,cam_front_translation,matrix_to_rotation_6d(quaternion_to_matrix(cam_front_rotation)),gt_trajectory[i]],dim=-1).unsqueeze(0)
        ...
        out['future_trajectory'] = gt_trajectory[self.movie_len:]
        return out
