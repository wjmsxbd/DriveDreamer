...
class dataloader(data.Dataset):
    def __init__(self,cfg,num_boxes,cond_stage_config,split_name='train'):
        ...
        self.cfg = cfg
        self.nusc = NuScenes(version=cfg['version'],dataroot=cfg['dataroot'])
    ...
    def load_idx(self,idx):
        if self.now_scene_sample_token == None:
            ...
            self.nusc_map = self.nusc_maps[self.scene_log['location']]
        out = {}
        now_reference_image,now_3dbox,now_hdmap,category,yaw,translation = get_this_scene_info(cfg['dataroot'],self.nusc,self.nusc_map,self.now_scene_sample_token)
