...
from torch.utils import data
from utils.tools import get_3dbox_in_image,get_this_scene_info,get_this_scene_info_with_lidar,get_this_scene_info_with_lidar_MV,get_global_pose,quaternion_to_matrix,matrix_to_rotation_6d
...
class dataloader(data.Dataset):
    def __init__(self,cfg,num_boxes,movie_len,sigma_sampler_config,split_name='train',return_pose_info=False,collect_condition=None,use_original_action=True):
        ...
        box_category = ['movable_object.barrier','vehicle.bicycle','vehicle.bus.bendy','vehicle.bus.rigid','vehicle.car','vehicle.construction','vehicle.motorcycle','human.pedestrian.adult','human.pedestrian.child','human.pedestrian.construction_worker','human.pedestrian.police_officer','movable_object.trafficcone','vehicle.trailer','vehicle.truck']
        self.map_category2color = {}
        colormap = plt.cm.get_cmap('jet', len(box_category))
        for i in range(len(box_category)):
            color = colormap(i)
            color_rgb = tuple(int(c*255) for c in color[:3])
            self.map_category2color[box_category[i]] = color_rgb
    ...
    def get_cam_token(self,sample_token):
        sample_record = self.nusc.get('sample',sample_token)
        cam_front_token = sample_record['data']['CAM_FRONT']
        return cam_front_token
    ...
    def get_data_info(self,idx,list_idx):
        ...
        for i in range(self.movie_len):
            ...
            cam_token = self.get_cam_token(sample_token)
            boxes = get_3dbox_in_image(cam_token,self.nusc,tuple(self.cfg['img_size']),self.map_category2color)
            boxes = torch.from_numpy(boxes.transpose(2,0,1))
            out['image'] = boxes
        return out
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config',
                        default='configs/process_nusc_pics.yaml',
                        type=str,
                        help="config path")
    ...
    box_save_path = os.path.join(save_path,'3Dbox')
    rank = 0
    if rank == 0:
            ...
            for _,batch in tqdm(enumerate(data_loader_)):
                if batch['first_frame'][0] == 1 or batch['first_frame'][0] == [1]:
                    ...
                    for i in range(len(first_frame_idx)):
                        ...
                        save_tensor_as_image(batch['image'][i],file_path=box_save_path,index=idx,frame=now_frames)
