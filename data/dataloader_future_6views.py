...
from torch.utils import data
from utils.tools import get_this_scene_info,get_this_scene_info_with_lidar,get_global_pose,quaternion_to_matrix,matrix_to_rotation_6d,get_bev_box_label,get_bev_hdmap_front_view,rotation_6d_to_matrix
...
class dataloader(data.Dataset):
    ...
    def get_data_info(self,idx):
        ...
        out['image'] = torch.zeros((self.movie_len,self.num_cameras,3,self.cfg['img_size'][1],self.cfg['img_size'][0]))
        out['intrinsics'] = torch.zeros((self.movie_len,self.num_cameras,3,3))
        out['ego2cam'] = torch.zeros((self.movie_len,self.num_cameras,4,4))
        ...
        out['global2ego'] = torch.zeros((self.movie_len,self.num_cameras,4))
        ...
        for i in range(self.movie_len):
            ...
            bev_hdmap = get_bev_hdmap_front_view(sample_token,self.nusc,nusc_map,img_size=(self.cfg['hdmap_size'][0],self.cfg['hdmap_size'][1]))
            ...
            bev_instance_label,bev_box = get_bev_box_label(sample_token,self.nusc,nusc_map,img_size=(self.cfg['hdmap_size'][0],self.cfg['hdmap_size'][1]),instance_label=self.instance_label)
            # bev_box = self.crop_image(bev_box,self.cfg['hdmap_size'][1],self.cfg['hdmap_size'][0],self.cfg['hdmap_size'][0]//2)
            
            instance_mask = bev_box[1].copy().astype(np.bool_) | bev_box[2].copy().astype(np.bool_)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config',
                        default='configs/MILE_6view.yaml',
                        type=str,
                        help="config path")
